...
export class AdminService {
  ...
  constructor(private http:HttpClient) { 
    this.url = "http://localhost:8090/admin/"
  }
...
public adminCount():Observable<number>{
  return this.http.get<number>('http://localhost:8090/admins/count');
}
public userCount():Observable<number>{
  return this.http.get<number>('http://localhost:8090/users/count');
}
public admins():Observable<Admin[]>{
  return this.http.get<Admin[]>('http://localhost:8090/admins');
}
...
public getName(address:string):Observable<String>{
  return this.http.get<String>('http://localhost:8090/user/name/'+address);
}
public getOrders():Observable<Orders[]>{
  return this.http.get<Orders[]>('http://localhost:8090/orders');
}
public getCount():Observable<number>{
  return this.http.get<number>('http://localhost:8090/orders/count');
}
public getValue():Observable<number>{
  return this.http.get<number>('http://localhost:8090/orders/value');
}
}
